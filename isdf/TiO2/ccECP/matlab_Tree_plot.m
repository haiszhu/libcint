...
for j=1:numel(ids)
  ...
  if (f.level(ids(j)) < height-9) && (-cenj(2)+cenj(3)<1 || cenj(1)>3.5 )
  % if (f.level(ids(j)) < height-9) && (-cenj(2)+cenj(3)<1 )
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-9) && (-cenj(2)+cenj(3)<0.5 || cenj(1)>4 )
  % if (f.level(ids(j)) == height-3)
    ids2 = [ids2 ids(j)];
  end
end
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-8) && -cenj(2)+cenj(3)<0.25
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-7) && -cenj(2)+cenj(3)<0.1
...
% line('XData', xdata(:), 'YData', ydata(:), 'ZData', zdata(:), 'LineWidth', 1, 'Color', 'g')
line('XData', xdata(:), 'YData', ydata(:), 'ZData', zdata(:), 'LineWidth', myLineWidth, 'Color', [0.4660, 0.6740, 0.1880]	)
axis equal, hold on
...
for j=1:numel(ids)
  ...
  if (f.level(ids(j)) == height-6) && -cenj(2)+cenj(3)<0.05
    ids5 = [ids5 ids(j)];
  end
end
...
line('XData', xdata(:), 'YData', ydata(:), 'ZData', zdata(:), 'LineWidth', myLineWidth, 'Color', 'r')
...
line('XData', xdata(:), 'YData', ydata(:), 'ZData', zdata(:), 'LineWidth', myLineWidth, 'Color', [0.25 0.25 0.25])
axis equal tight, hold on
...
view(0,0)
% title('Planar view of the octree for (NH_3)_2', 'FontSize',14)
exportgraphics(gcf, 'octreeplanar.pdf','contenttype','vector');
