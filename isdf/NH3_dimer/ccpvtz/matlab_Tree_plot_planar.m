...
for j=1:numel(ids)
  ...
  if (f.level(ids(j)) < height-4) && cenj(2)>-0.1
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-4) && cenj(2)>-0.05
  % if (f.level(ids(j)) == height-3)
    ids2 = [ids2 ids(j)];
  end
end
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-3) && cenj(2)>-0.025
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-2) && cenj(2)>-0.2
...
for j=1:numel(ids)
  ...
  if (f.level(ids(j)) == height-1) && cenj(2)>-0.01
    ids5 = [ids5 ids(j)];
  end
end
