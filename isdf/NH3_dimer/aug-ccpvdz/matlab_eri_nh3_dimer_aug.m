...
if 1
  load('nh3_dimer_aug_basis_check.mat')
  [x2, y2, z2] = ndgrid(linspace(0,1,5), linspace(0,1,5), linspace(0,1,5));
  xyz2 = [x2(:) y2(:) z2(:)];
  func = @(x,y,z) cgtofunc_nh3_dimer_aug_ccpvdz(x,y,z);
  vals2 = func(x2, y2, z2);
  vals2rs = reshape(vals2,[],100);
  diff = abs(vals - reshape(vals2rs,[],100));
  max(diff(:))
end
...
func2 = @(x,y,z) cgto2func_nh3_dimer_aug_ccpvdz(x,y,z);
% func2 = @(x,y,z) cgtofunc_nh3_dimer_aug_ccpvdz(x,y,z);
checkpts = [
...
h5_filename = sprintf('src_nh3_dimer_aug_ccpvdz_%s.h5', eps_str);
...
%%% save data
eps_str = sprintf('%.0e', eps);
mat_filename = sprintf('ERI_nh3_dimer_aug_ccpvdz_%s.mat', eps_str);
h5_filename = sprintf('ERI_nh3_dimer_aug_ccpvdz_%s.h5', eps_str);
