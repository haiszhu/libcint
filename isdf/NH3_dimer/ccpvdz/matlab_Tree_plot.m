...
for j=1:numel(ids)
  ...
  if (f.level(ids(j)) < height-3) && (-cenj(2)+cenj(3)<1 || cenj(1)>3.5 )
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-3) && (-cenj(2)+cenj(3)<0.5 || cenj(1)>4 )
  % if (f.level(ids(j)) == height-3)
    ids2 = [ids2 ids(j)];
  end
end
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-2) && -cenj(2)+cenj(3)<0.25
...
for j=1:numel(ids)
  cenj = 1/2*(f.domain(1:2:5,ids(j))+f.domain(2:2:6,ids(j)));
  if (f.level(ids(j)) == height-1) && -cenj(2)+cenj(3)<0.1
