...
bas_string = extractAfter(basname, 'cc-');
[~, idbasename, ~] = fileparts(idfname);
eps_string = extractAfter(idbasename, '_'); 

% save
mat_filename = ['Vmunu_h2o_cc' bas_string '_eps_' eps_string '.mat'];
save(mat_filename, 'Vmunu');
h5_filename = ['Vmunu_h2o_cc' bas_string '_eps_' eps_string '.h5'];
h5create(h5_filename,"/DS1",[nd nd])
h5write(h5_filename,"/DS1",Vmunu)
h5disp(h5_filename)

% save
eri_mat_filename = ['ERI_h2o_cc' bas_string '_eps_' eps_string '.mat'];
save(eri_mat_filename,'Vijkl')    
eri_h5_filename = ['ERI_h2o_cc' bas_string '_eps_' eps_string '.h5'];
