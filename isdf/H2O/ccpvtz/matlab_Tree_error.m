...
maxvals    = zeros(size(epsvals));
maxval2s   = zeros(size(epsvals));
maxval2ups = zeros(size(epsvals));
for i = 1:numel(epsvals)
  ...
  [relerr,maxval] = treedata_error(f,func);
  ...
  [relerr2,maxval2] = treedata_error(f2,func2);
  disp("========= Relative L infinity error on order p grid =======");
  ...
  [relerr2up,maxval2up] = treedata_error(f2up,func2);
...
figure(1),clf,
loglog(epsvals,relerrs,'o-')
hold on
loglog(epsvals,relerr2s,'^-')
loglog(epsvals,relerr2ups,'s-')
