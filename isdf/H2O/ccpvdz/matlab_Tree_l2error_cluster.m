...
for i = 1:numel(epsvals)
  ...
  [relerr,maxval] = treedata_l2error(f,func);
  ...
  [relerr2,maxval2] = treedata_resample_l2error(f,func2,treefun_order);
  disp("========= Relative L^2 error on order p grid =======");
  ...
  upfactor = 1.5;
  [relerr2up,maxval2up] = treedata_resample_l2error(f,func2,ceil(upfactor*treefun_order));
  disp("========= Relative L^2 error on order 1.5*p grid =======");
