...
void GTOieval_loop(void (*fiter)(), FPtr_eval feval, FPtr_exp fexp, double fac,
...
#pragma omp parallel
{
        ...
        int ncart = NCTR_CART * param[TENSOR] * param[POS_E1];
        double *buf = malloc(sizeof(double) * BLKSIZE*(NPRIMAX*2+ncart+1));
        double *ao_buf = malloc(sizeof(double) * nao * BLKSIZE);
#pragma omp for schedule(dynamic, 4)
        for (k = 0; k < nblk*nshblk; k++) {
                iloc = k / nblk;
                ish = shloc[iloc];
                ...
                bgrids = MIN(ngrids-ip, BLKSIZE);
                (*fiter)(feval, fexp, fac, nao, Ngrids, bgrids,
                         param, shloc+iloc, ao_loc, buf, ao_buf+aoff*Ngrids+ip,
                         coord+ip, non0table+ib*nbas,
                         atm, natm, bas, nbas, env);
        }
        for (int j = 0; j < ngrids; j++) {
          aoi[j] = ao_buf[j * nao + (i - 1)];
        }
        free(ao_buf);
        free(buf);
}
...
void GTOieval_sph_drv(FPtr_eval feval, FPtr_exp fexp, double fac, int i, int ngrids,
                        int param[], int *shls_slice, int *ao_loc,
                        double *aoi, double *coord, uint8_t *non0table,
                        int *atm, int natm, int *bas, int nbas, double *env)
{
        ...
        // }
        // free(ao);
        GTOieval_loop(GTOeval_sph_iter, feval, fexp, fac, i, ngrids,
                param, shls_slice, ao_loc,
